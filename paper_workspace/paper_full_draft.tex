...
\begin{description}
	...
	\item [Sea conditions]    The sea conditions  are given by the wind speed and direction, sea current speed and direction, and wave condition measured in Beaufort units. In reality these conditions change continuously in time and space but in our model they are assumed fixed for each discretized time and space unit. The time, for this end, is discretized to \emph{blocks} of six hours and the space to \emph{cells} of $0.5\degree \times 0.5\degree$ latitude and longitude. Recall that each latitude degree and each longitude degree at the equator is 60 NM (111 km).  This discretization level was chosen because it is used by global weather and forecast services.  %The forecast is given for each such time and space discrerized unit for an horizon of seven days and is updated every six hours. 
...
\caption{An initial state and its candidate successors in one real interior block of the Indian
Ocean route, voyage~0 ($d\in[1963.886,1995.719]$ NM, $t\in[120,126]$ h). The source is
$(121\ \mathrm{h},1963.886\ \mathrm{NM})$ in (a) and $(120\ \mathrm{h},1970\ \mathrm{NM})$ in (b).
Circles are states on the distance lines, which carry the $\tau$-grid; squares are states on the
time lines, which carry the $\delta$-grid; both grids are anchored at the voyage start
(Eq.~\eqref{eq:state-space}). Coloured markers are the successors admitted by
Eq.~\eqref{eq:neighbors}, grey ones the remaining states. The cone spans the $V_{\min}$ and
$V_{\max}$ rays; with $V_{\min}>0$ it has no vertical edge, since Eq.~\eqref{eq:speed-set} admits
the zero-speed wait leg only at $d=0$ and $d=D$. For legibility the drawing uses $\delta=5$ NM,
$\tau=1$ h and $[V_{\min},V_{\max}]=[3,16]$ kn; the experiments use $\delta=1$ NM, $\tau=0.1$ h and
the band of Section~\ref{sec:data}.}
...
high-resolution configuration rather than with GFS at $0.25\degree$. Re-initialised every 6 h, retrieved
to a 168 h horizon.
\item \textbf{Waves} (significant wave height) --- the M\'et\'eo-France wave model (MFWAM), a
third-generation spectral ocean-wave model, at $0.08\degree$ resolution, re-initialised twice daily,
retrieved to a 168 h horizon.
...
re-plan cadence used later. The wind product refreshes every 6 h, MFWAM (waves) every 12 h, and
