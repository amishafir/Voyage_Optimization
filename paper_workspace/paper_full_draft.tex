...
Let $\mathcal{D}(d) = \arg\min_{i} \{i \in \mathbb{N} : d_i > d\}$ define the index of of the distance line that follows distance $d$. Similary $\mathcal{T}(t) = \arg\min_{j} \{j \in \mathbb{N} : t_j > t\}$.  Now for any state $(d,t)  \in \mathcal{S}$ we define the set of neighboring states \[ \mathcal{A}(d,t)  = \biggl \{ (d_{\mathcal{D}(d)},t') : t' = t_{\mathcal{T}(t)} - n \tau, n=0,\ldots, 	\left\lfloor\frac{t_{\mathcal{T}(t)}-t_{\mathcal{T}(t)-1}}{\tau}\right\rfloor   \biggr\}  \]
