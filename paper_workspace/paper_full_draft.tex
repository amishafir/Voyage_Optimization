...
\biggl\{ (d_{\mathcal{D}(d)},\, t') : t' = t_{\mathcal{T}(t)} - n\tau,\;
   n=0,\ldots,\left\lfloor\frac{t_{\mathcal{T}(t)}-t_{\mathcal{T}(t)-1}}{\tau}\right\rfloor \biggr\} \cup \\
&\biggl\{ (d',\, t_{\mathcal{T}(t)}) : d' = d_{\mathcal{D}(d)} - n\delta,\;
   n=0,\ldots,\left\lfloor\frac{d_{\mathcal{D}(d)}-d_{\mathcal{D}(d)-1}}{\delta}\right\rfloor \biggr\}.
\end{aligned}
\end{equation}
...
C^{*}(d,t)=\min_{\substack{(\tilde d,\tilde t)\in\mathcal{S}\,:\,(d,t)\in\mathcal{A}(\tilde d,\tilde t)\\ \bar v \,=\, (d-\tilde d)/(t-\tilde t)\,\in\,\mathcal{V}}}
\Bigl[\, C^{*}(\tilde d,\tilde t)+(t-\tilde t)\,\phi(\tilde d,\tilde t;\bar v)\,\Bigr].
\end{equation}
With the boundary condition 
$$C^*(0,0) = 0.$$
The solution of this Bellman equation gives the minimum fuel to arrive at each state $(d,t)$; for each state we record the minimising predecessor and the realised leg speed $\bar v=\Delta d/\Delta t$ of the incoming arc, from which the optimal speed schedule is recovered by back-tracking to the origin.

% ==== OLD (argmin over finite speed set V) — superseded by argmin over 𝒱; kept, not rendered ====
\begin{comment}
\begin{equation}
...
\end{equation}
\end{comment}
% ==== OLD (argmin over the continuous interval 𝒱, two cases) — superseded by node-first; kept, not rendered ====
\begin{comment}
\begin{equation}
V^{*}(d,t)=\arg\min_{v \in \mathcal{V}}  \begin{cases}  C^*(d-v(t-\underline{t}), \underline{t})+(t-\underline{t})\phi(\underline{d}, \underline{t}; v) & \text{if }  \frac{d- \underline{d}}{v}  \ge t-\underline{t}, \\
	C^*\left(\underline{d}, 	t- \frac{d -\underline{d}}{ v}\right)
	+ \frac{d -\underline{d}}{ v}\phi(\underline{d}, \underline{t}; v) & \text{otherwise.}
\end{cases}
\end{equation}
...
\end{center}
\end{comment}
% ==== NEW Algorithm 1 (node-first: enumerate reachable far-wall grid nodes) ====
\begin{center}
\fbox{\begin{minipage}{0.95\textwidth}
...
$\mathcal{S}\leftarrow\{(0,0)\}$;\quad $\mathcal{A}\leftarrow\emptyset$;\quad queue $Q\leftarrow[(0,0)]$\\
\textbf{while} $Q$ not empty \textbf{do}\\
\> $(d,t)\leftarrow \mathrm{pop}(Q)$;\quad \textbf{if} $d=L$ \textbf{or} $t\ge T$ \textbf{then continue}\\
\> $\overline{d}\leftarrow\min\{d_i>d\}$;\quad $\overline{t}\leftarrow\min\{t_j>t\}$;\quad $C\leftarrow\emptyset$ \quad{\small$\triangleright$ candidate successors $(d',t')$}\\
