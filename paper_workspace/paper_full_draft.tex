...
The rectangular areas defined by the vertical and horizontal lines are sub-spaces of the distance--time plane. Each point within such a sub-space  represent a location and time in which the heading of the vessel and the see conditions are assumed fixed. Threfore, the optimal speed of the vessel is constant. The only points at which the vessel should make a decision lie on the vertical and horizontal lines. We approximate the optimal speed in each sub-space using a disceret dynamic program. For this end we discretized the vertical distance line to a points every fixed time interval (e.g., $\tau = 0.1$ hours.  Similarly the horizontal time lines are discretized to distance interval of fixed length(e.g., $\delta = 1$ NM), starting from the crossing of each time line with a distance line and going bacward in space.   ADD FIGURE. 
...
of $\delta>0$; the time operator $\langle\cdot\rangle_{\tau}$ is defined identically with step $\tau$ in
place of $\delta$. When an arc terminates on a time line, its free coordinate (distance) is snapped to a
grid of step $\delta$; when it terminates on a distance line, its free coordinate (time) is snapped to a
grid of step $\tau$. We use $\delta=1$ NM and $\tau=0.1$ h. % ==== OLD (snap merges distinct-speed arrivals; arc bound in |V|) — superseded; kept, not rendered ====
\begin{comment}
