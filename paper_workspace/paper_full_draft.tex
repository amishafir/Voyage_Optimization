...
% ==== fig:state-neighbours: matched two-panel asset generated from one real Route-1 block. ====
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/state_neighbours_pair.pdf}
\caption{An initial state and its candidate successors, on a schematic block: a $5$ NM
subsegment, $d\in[240,245]$ NM, inside a six-hour epoch, $t\in[24,30]$ h, with each panel
showing a $2.3$ h window of it. The coordinates are illustrative, chosen so the grids can be read off
the axes; $5$ NM is representative of a $0.08\degree$ cell on these routes. The source is
$(24.2\ \mathrm{h},240\ \mathrm{NM})$ in (a) and $(29\ \mathrm{h},240\ \mathrm{NM})$ in (b). Every state is drawn as a disc: those on the
distance lines carry the $\tau$-grid, those on the time lines the $\delta$-grid, and both grids are
anchored at the voyage start (Eq.~\eqref{eq:state-space}). Blue marks the successors admitted by
Eq.~\eqref{eq:neighbors} on the next distance line, green those admitted on the next time line, and
grey the remaining states. The cone spans the
$V_{\min}$ and $V_{\max}$ rays; with $V_{\min}>0$ it has no vertical edge, since
Eq.~\eqref{eq:speed-set} admits the zero-speed wait leg only at $d=0$ and $d=D$. At this cell size
the vessel normally leaves through the distance wall, as in (a); the time wall is reachable only late
in the epoch, as in (b). The drawing uses the experimental grids, $\delta=0.2$ NM and $\tau=0.1$ h,
with the band widened to $[3,16]$ kn for legibility.}
