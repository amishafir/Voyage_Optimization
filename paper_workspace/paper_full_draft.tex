...
Given the input above a solution of the deterministic speed control problem is the SOG of the vessel at any given time during the voyage (denoted $v(t)$) so as to minimize the total fuel consumption of the voyage will satisfying the ETA requirement.    The voyage can be divided into legs during which the course and and the weather conditions are fixed.  Due to the convexity of the FCR function,  in an optimal solution the SOG is fixed during each such leg.  The SOG of the vessel is seleceted from a bounded interval, $\mathcal{V}=[v_{min}, v_{max}]$ knots ($NM/h$). Therefore, a solution can be described by a list of such legs and the SOG in each one of them.
...
\end{comment}
% ==== NEW (2026-08-04 flat rewrite: one section, one claim — build+discretise+price in a single pass, then one backward sweep) ====
Section~\ref{sec:states} supplies every mathematical object the solution requires: the state grid
$\mathcal{S}$ (Eq.~\eqref{eq:state-space}), the successor set $\mathcal{A}(d,t)$ of each state
(Eq.~\eqref{eq:neighbors}), the fuel-to-go recursion that couples them
(Eq.~\eqref{eq:cost-to-arrive}) with its boundary condition, and the optimal outgoing speed
...
\textbf{Algorithm~1: Build the reachable graph, backpropagate, and extract the optimal schedule}\\[-6pt]
...
{\small$\triangleright$ \emph{Stage 1 --- forward closure: discover the reachable states and price every arc}}\\
{\scriptsize 1}\> $\mathcal{S}\leftarrow\{(0,0)\}$;\quad $\mathcal{A}\leftarrow\emptyset$;\quad queue $Q\leftarrow[(0,0)]$\\
{\scriptsize 2}\> \textbf{while} $Q$ not empty \textbf{do}\\
{\scriptsize 3}\>\> $(d,t)\leftarrow \mathrm{pop}(Q)$;\quad \textbf{if} $d=L$ \textbf{or} $t= T$ \textbf{then continue}\\
{\scriptsize 4}\>\> $C\leftarrow\mathcal{A}(d,t)$ of Eq.~\eqref{eq:neighbors} \quad{\small$\triangleright$ arrival times on $d_{\mathcal{D}(d)}$, arrival distances on $t_{\mathcal{T}(t)}$; $\bar v\in\mathcal{V}$ built in}\\
\>\>\> {\small$\triangleright$ if $d_{\mathcal{D}(d)}$ is unresolvable on the $\tau$-grid ($\langle t+(d_{\mathcal{D}(d)}-d)/v_{\max}\rangle_\tau=t$), skip it: the time-line}\\
\>\>\> {\small\phantom{$\triangleright$ }candidates extend past it as far as $v_{\max}$ allows (glide rule, see below)}\\
{\scriptsize 5}\>\> \textbf{for} $(\tilde d,\tilde t)\in C$ \textbf{do}\\
...
{\scriptsize 8}\>\>\> \textbf{if} $(\tilde d,\tilde t)\notin\mathcal{S}$ \textbf{then} $\mathcal{S}\leftarrow\mathcal{S}\cup\{(\tilde d,\tilde t)\}$;\ $\mathrm{push}(Q,(\tilde d,\tilde t))$\\
{\scriptsize 9}\>\> \textbf{end for}\\
{\scriptsize 10}\> \textbf{end while}\\
{\small$\triangleright$ \emph{Stage 2 --- backpropagation: Eq.~\eqref{eq:cost-to-arrive} evaluated once per state, every successor already valued}}\\
{\scriptsize 11}\> $C^{*}(s)\leftarrow+\infty$ for all $s\in\mathcal{S}$;\quad $C^{*}(L,t)\leftarrow0$ for every $(L,t)\in\mathcal{S}$\\
{\scriptsize 12}\> \textbf{for} each $(d,t)\in\mathcal{S}$ in \emph{reverse} lexicographic $(t,d)$ order \textbf{do}\\
{\scriptsize 13}\>\> $C^{*}(d,t)\leftarrow\displaystyle\min_{(d,t)\to(\tilde d,\tilde t)\,\in\,\mathcal{A}}\bigl[\phi(d,t;\bar v)\,(\tilde t-t)+C^{*}(\tilde d,\tilde t)\bigr]$;\quad record the minimiser $(\tilde d^{\star},\tilde t^{\star})$\\
{\scriptsize 14}\> \textbf{end for}\\
{\small$\triangleright$ \emph{Stage 3 --- extraction}}\\
{\scriptsize 15}\> $F^{\star}\leftarrow C^{*}(0,0)$;\quad walk $(0,0)\to(\tilde d^{\star},\tilde t^{\star})\to\cdots$ until $d=L$, emitting $V^{*}(d,t)$ of Eq.~\eqref{eq:opt-speed}\\
{\scriptsize 16}\> \textbf{return} $F^{\star}$ and the speed schedule
\end{tabbing}
\end{minipage}}
\end{center}
...
To see the single pass at work (lines 1--10), consider a state
$(d,t)$ as the loop pops it (line~3): the vessel is $d$~NM from departure at elapsed time $t$,
...
on one of these two walls, and the grid picks out the nodes it can land on (line~4): on the distance
...
wall, every $\tau$-spaced grid point between the earliest arrival $t+(d_{\mathcal{D}(d)}-d)/v_{\max}$
and the latest arrival $t+(d_{\mathcal{D}(d)}-d)/v_{\min}$, clipped at the time wall and at $T$; on
the time wall, every $\delta$-spaced grid point between the shortest advance
$d+v_{\min}(t_{\mathcal{T}(t)}-t)$ and the longest $d+v_{\max}(t_{\mathcal{T}(t)}-t)$. These are
exactly the two families of Eq.~\eqref{eq:neighbors}. No speed set is ever introduced: each
candidate node fixes its own leg speed $\bar v=(\tilde d-d)/(\tilde t-t)$ (line~6), admissible by
construction, and because the node already sits on the grid, no target-versus-realised speed gap
% ==== OLD ("typically has κ≈8 such candidates" — audit item B7: κ was used before its definition) ====
...
% ==== FIG placeholder realised 2026-08-04 (figures/plot_state_neighbours.py) ====
...
where the forecast is refreshed. The speed cone $[v_{\min},v_{\max}]$ intersects the walls in two
windows, and the grid picks out the nodes inside them --- arrival times spaced $\tau$ on the
distance wall, arrival distances spaced $\delta$ on the time wall: the two families of
Eq.~\eqref{eq:neighbors}. The fast edge of the cone crosses the distance wall before the block ends
(capping the second family), the slow edge runs out of the block first (clipping the first), and
each surviving node fixes its own leg speed $\bar v=(\tilde d-d)/(\tilde t-t)$ --- here
$\kappa=8$ candidates in total.}
...
extending beyond the skipped line as far as $v_{\max}$ allows (the glide rule annotated under
line~4). Third, a candidate whose leg speed cannot be sustained in the prevailing sea conditions ---
its fuel rate $\phi(d,t;\bar v)$ undefined --- is discarded (line~6).
...
% ==== OLD opening ("Stage 2 ... answers the second question") — reworked for the flat flow ====
%Stage~2 (lines 11--14) answers the second question: \emph{what does each decision cost from there to
%the destination?} Every arc strictly increases both coordinates, so the graph
...
% ==== OLD heading (2026-08-04 flat rewrite) ====
%\subsubsection{Stage 3: reading off the plan}
%\label{sec:extract}
% ==== OLD opening ("Stage 3 ... answers the last question") — reworked for the flat flow ====
%Stage~3 (lines 15--16) answers the last question: \emph{which decisions do we take?} After the sweep,
...
terminates at $d=L$ (line~16).
% ==== OLD soft-ETA remark ("lateness charge λ(t̃−T)" — audit item A4: vacuous, t̃≤T on the grid) ====
%Replacing the $\infty$ case of Eq.~\eqref{eq:cost-to-arrive} with a lateness charge $\lambda\,(\tilde t-T)$ turns the hard deadline into a soft one ...
Extending the time lines beyond $T$ and replacing the $\infty$ case of
...
the forward closure (Stage~1) yields about $1.3\times10^{5}$ states and $1.2\times10^{6}$
arcs --- the node-first enumeration emits only the $\kappa\approx8$ distinct reachable far-wall nodes per
% ==== OLD ("the single forward sweep of Algorithm~2") — Algorithm 2 absorbed into Algorithm 1 (Stage 2) ====
%state rather than one arc per speed --- and the single forward sweep of Algorithm~2 solves it in
state rather than one arc per speed --- and the single backward sweep (Stage~2) solves it in
under two seconds; the
