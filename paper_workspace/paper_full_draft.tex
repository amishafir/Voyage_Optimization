...
% ==== NEW (cost-to-go: record the minimising successor; recover the schedule by walking forward from the origin) ====
...
\end{comment}
% ==== NEW (S and A are already defined by Eqs. 4-5; what remains is bounding, enumerating, sweeping) ====
...
% ==== OLD ("the enumeration and the sweep as pseudocode" — two boxes) — superseded by the consolidated Algorithm 1; kept, not rendered ====
%give the enumeration and the sweep as pseudocode.
% ==== NEW (one pseudocode: closure + backpropagation + extraction) ====
...
\end{comment}
% ==== NEW (grid already fixed by Eq. 4; the operators below are the rounding arithmetic onto it) ====
...
and $\tau=0.1$ h.% ==== OLD (snap merges distinct-speed arrivals; arc bound in |V|) — superseded; kept, not rendered ====
\begin{comment}
...
\end{comment}
% ==== NEW (bound follows directly from counting Eq. 4; arc bound in K) ====
...
\end{comment}
% ==== NEW (d̄/t̄ are shorthand for §4.1's index maps) ====
...
index maps $\mathcal{D},\mathcal{T}$ of Section~\ref{sec:states}.% ==== OLD (iterate over speeds v in V, hold until a line) — superseded; kept, not rendered ====
\begin{comment}
...
\end{comment}
% ==== NEW (enumerate the reachable grid nodes on the two lines directly) ====
...
% ==== OLD ("as the predecessor (d̃,t̃) does" — stale after the cost-to-go flip: the rectangle is now fixed by (d,t) itself in Eq. 6) ====
%as the predecessor $(\tilde d,\tilde t)$ does in
as it does in
...
\end{center}
\end{comment}
% ==== NEW Algorithm 1 (consolidated, cost-to-go): Stage 1 forward closure = states + priced arcs;
% ==== Stage 2 backpropagation = Eq. 6 once per state; Stage 3 = two-line extraction ====
\begin{center}
\fbox{\begin{minipage}{0.95\textwidth}
...
\textbf{while} $Q$ not empty \textbf{do}\\
\> $(d,t)\leftarrow \mathrm{pop}(Q)$;\quad \textbf{if} $d=L$ \textbf{or} $t= T$ \textbf{then continue}\\
\> $C\leftarrow\mathcal{A}(d,t)$ of Eq.~\eqref{eq:neighbors} \quad{\small$\triangleright$ arrival times on $d_{\mathcal{D}(d)}$, arrival distances on $t_{\mathcal{T}(t)}$; $\bar v\in\mathcal{V}$ built in}\\
...
\> \textbf{for} $(\tilde d,\tilde t)\in C$ \textbf{do}\\
\>\> $\bar v\leftarrow(\tilde d-d)/(\tilde t-t)$;\quad \textbf{if} $\bar v$ unattainable at $(d,t)$ \textbf{then continue}\\
\>\> $\mathcal{A}\leftarrow\mathcal{A}\cup\bigl\{\,(d,t)\!\to\!(\tilde d,\tilde t)\ \text{with speed}\ \bar v,\ \text{cost}\ \phi(d,t;\bar v)\,(\tilde t-t)\,\bigr\}$\\
\>\> \textbf{if} $(\tilde d,\tilde t)\notin\mathcal{S}$ \textbf{then} $\mathcal{S}\leftarrow\mathcal{S}\cup\{(\tilde d,\tilde t)\}$;\ $\mathrm{push}(Q,(\tilde d,\tilde t))$\\
\> \textbf{end for}\\
\textbf{end while}\\
...
$C^{*}(s)\leftarrow+\infty$ for all $s\in\mathcal{S}$;\quad $C^{*}(L,t)\leftarrow0$ for every $(L,t)\in\mathcal{S}$\\
\textbf{for} each $(d,t)\in\mathcal{S}$ in \emph{reverse} lexicographic $(t,d)$ order \textbf{do}\\
\> $C^{*}(d,t)\leftarrow\displaystyle\min_{(d,t)\to(\tilde d,\tilde t)\,\in\,\mathcal{A}}\bigl[\phi(d,t;\bar v)\,(\tilde t-t)+C^{*}(\tilde d,\tilde t)\bigr]$;\quad record the minimiser $(\tilde d^{\star},\tilde t^{\star})$\\
\textbf{end for}\\
{\small$\triangleright$ \emph{Stage 3 --- extraction}}\\
$F^{\star}\leftarrow C^{*}(0,0)$;\quad walk $(0,0)\to(\tilde d^{\star},\tilde t^{\star})\to\cdots$ until $d=L$, emitting $V^{*}(d,t)$ of Eq.~\eqref{eq:opt-speed}\\
\textbf{return} $F^{\star}$ and the speed schedule
\end{tabbing}
\end{minipage}}
\end{center}
...
\end{center}
\end{comment}
% ==== NEW (cost-to-go: reverse-topological sweep = Stage 2 of Algorithm 1; extraction at the origin = Stage 3) ====
Every arc strictly increases both coordinates ($\Delta d>0$ and $\Delta t>0$), so the graph
...
the closure of Algorithm~1 yields about $1.3\times10^{5}$ states and $1.2\times10^{6}$
...
state rather than one arc per speed --- and the single backward sweep (Stage~2 of Algorithm~1) solves it in
