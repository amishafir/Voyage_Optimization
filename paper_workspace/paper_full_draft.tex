...
\end{enumerate}
The two routes are depicted in Figure~\ref{fig:routes} and their waypoints are detailed in Table~\ref{tab:waypoints}.  The speed optimization problem can be solved with various ETA values. Moreover, because the sea conditions change over time, a problem instance is determined by the  route, departure time and desired ETA.  In our experiment we tested each route with 25 different departure times during the spring of 2026, spaced three days apart.  Each of these $2 \times 25$ route-departure time combinations were tested with 3 ETA levels corresponding to an average sailing speed of 10, 12, and 14 NM, making a benchmark set consisting of 150 problem instances. 
