...
(Algorithm~1).
\end{comment}
% ==== NEW (2026-08-04 restructure: box first, then a walkthrough whose subsections mirror the three stages) ====
...
{\small$\triangleright$ \emph{Stage 1 --- forward closure: discover the reachable states and price every arc (Section~\ref{sec:enumerate})}}\\
...
\>\>\> {\small\phantom{$\triangleright$ }candidates extend past it as far as $v_{\max}$ allows (glide rule, Section~\ref{sec:enumerate})}\\
...
{\small$\triangleright$ \emph{Stage 2 --- backpropagation: Eq.~\eqref{eq:cost-to-arrive} evaluated once per state, every successor already valued (Section~\ref{sec:sweep})}}\\
...
{\small$\triangleright$ \emph{Stage 3 --- extraction (Section~\ref{sec:extract})}}\\
...
wall, every $\tau$-multiple between the earliest arrival $t+(d_{\mathcal{D}(d)}-d)/v_{\max}$ and the
latest arrival $t+(d_{\mathcal{D}(d)}-d)/v_{\min}$, clipped at the time wall and at $T$; on the time
wall, every $\delta$-multiple between the shortest advance $d+v_{\min}(t_{\mathcal{T}(t)}-t)$ and the
longest $d+v_{\max}(t_{\mathcal{T}(t)}-t)$. These are exactly the two families of
Eq.~\eqref{eq:neighbors}. No speed set is ever introduced: each candidate node fixes its own leg
speed $\bar v=(\tilde d-d)/(\tilde t-t)$ (line~6), admissible by construction, and because the node
already sits on the grid, no target-versus-realised speed gap arises. For our grid a state typically
has $\kappa\approx8$ such candidates. \todo{FIG: a single state $(d,t)$, its two walls
...
\label{sec:extract}
% ==== NEW (2026-08-04 restructure: Stage 3 walkthrough, lines 15-16) ====
Stage~3 (lines 15--16) answers the last question: \emph{which decisions do we take?} After the
sweep, $C^{*}(d,t)$ holds the minimum fuel-to-go from every reachable state, so the optimal voyage
fuel is read directly at the origin (line~15),
\begin{equation}
\label{eq:opt-fuel}
F^{\star} \;=\; C^{*}(0,0),
\end{equation}
...
terminates at $d=L$ (line~16). Replacing the $\infty$ case of Eq.~\eqref{eq:cost-to-arrive} with a
lateness charge $\lambda\,(\tilde t-T)$ turns the hard deadline into a soft one that prices each
hour of late arrival at $\lambda$ units of fuel and recovers the hard constraint as
$\lambda\to\infty$; all results in this paper use the hard constraint.
...
How large a graph can Stage~1 hand to Stage~2? Counting Eq.~\eqref{eq:state-space}, each distance
