...
trajectory from $(0,0)$ to the destination distance $(L,t)$ for some $t\leq T$, with instantaneous slope equal $v(t)$. 
...
$t_{\mathcal{T}(t)}$, with the arrival distance snapped to the $\delta$-grid.  We define the inverse set function $\mathcal{A}(d,t)^{-1} \equiv \{(\tilde d, \tilde t) : (\tilde d, \tilde t) \in \mathcal{A}(d,t)  \}$. That is the set of all previous discretized location in the time-distance space from which $(d,t)$ can be reached. 
...
C^{*}(d,t)=\min_{\substack{(\tilde d,\tilde t) \in\mathcal{A}^{-1}( d, t)}}   \begin{cases}     \infty & \tilde t =  t_{\Theta} , d < d_M \\
	C^{*}(\tilde d,\tilde t)+(\tilde t- t)\,\phi(d,t; (\tilde d- d)/(\tilde t-t))  & \text{otherwise}
\end{cases}		
\end{equation}
With the boundary condition $C^*(0,0) = 0$ and the the minimum total voyage fuel is obtained at $C^{*}(L,T)$.
...
For each state $(d,t)$  the algorithm scans the discretized reachable states $(\tilde d, \tilde t)  \in \mathcal{A}(d,t)$. 
Each candidate state arrives already discretised. The leg speed
$\bar v=(\tilde d-d)/(\tilde t-t)$ implied by the state transition and priced on the spot at $\phi(d,t;\bar v)\,(\tilde t-t)$. The value of the endpoint $(\tilde d, \tilde t)$ is
...
Extraction of the actual journeyis then preformed (lines~10--15).  The voyage terminates at $(L,T)$, possibly with some waiting at distance $L$ so an early arrival continues to the deadline at
...
{\scriptsize 7}\>\>\>\> $C^{*}(\tilde d,\tilde t) \leftarrow C^*(d,t)+ \phi(d,t;(\tilde d-d)/(\tilde t-t))\,(\tilde t-t)$; \quad{\small$\triangleright$ update state value, see \eqref{eq:cost-to-arrive}}\\
...
{\scriptsize 10}\> $curr \leftarrow (L,T)$; \\ 
