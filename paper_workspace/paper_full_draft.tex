...
programs used for fleet and routing problems \citep{Bektas2011, Wang2012}. These models treat
conditions as constant per segment and optimise a single speed accordingly. When the environment
varies in time and space the speed decision must be resolved more finely, which motivates graph and
dynamic-programming formulations over a space--time grid \citep{Zaccone2018} and fuel models that
distinguish still-water speed from speed over ground \citep{yang2020}.

The state of the art for fuel-minimising speed control under varying weather is the multistage-graph
formulation of \citet{luo2024}, which we adopt as our principal baseline. There the voyage is divided
into stages equal to the meteorological forecast cycle; each stage is traversed at a single constant
speed, the stage nodes discretise the remaining distance to the destination, and the minimum-fuel
path is found by shortest path and re-solved as updated forecasts arrive. The defining characteristic
is that \emph{speed is held constant across an entire forecast-cycle stage} --- the construction
resolves the speed decision only to that coarse stage. This is structural rather than incidental: the
number of candidate speed profiles grows as $K^{N}$ in the number of stages $N$ (with $K$ discrete
speed levels), and the formulation keeps stages coarse precisely to hold that combinatorial cost in
check. Refining it to the resolution at which the weather actually varies --- the native grid of the
forecast product, several cells per stage --- would require stages defined by space rather than by
time, whose durations then depend on the chosen speed, taking the model outside its fixed-duration
multistage structure and inflating the very profile space it exists to contain. Prior formulations
therefore cannot reach a per-cell speed resolution tractably. Our first contribution is a dynamic
program that does: speed is free to change at every cell boundary, yet the problem is solved by a
single pass over a directed acyclic graph.
