...
where the operative data change --- each $0.08\degree$ weather-cell crossing and each heading change ---
...
\begin{description}
	...
	\item [Sea conditions]    The sea conditions are given by the wind speed and direction, summarised by the Beaufort number, and by the sea current speed and direction. In reality these conditions change continuously in time and space but in our model they are assumed fixed for each discretized time and space unit. The time, for this end, is discretized to \emph{blocks} of six hours and the space to \emph{cells} of $0.08\degree \times 0.08\degree$ latitude and longitude. Recall that a degree of latitude is 60 NM (111 km) everywhere, while a degree of longitude is 60 NM at the equator and shrinks with the cosine of the latitude; a $0.08\degree$ cell is therefore roughly 5 NM across on the routes studied here. This discretization level was chosen because it is the native resolution at which the global weather and forecast services used in this study deliver their fields (Section~\ref{sec:data}).  %The forecast is given for each such time and space discrerized unit for an horizon of seven days and is updated every six hours. 
...
As a preprocessing step we divide each segment of the vessel route into \emph{subsegments}. Each time the route crosses a $0.08\degree$ latitude or longitude line, a new subsegment starts, so a subsegment is about 5 NM long. Within each time block, the sea conditions are assumed constant throughout a subsegment but they may change when the vessel enters a new sea cell. In addition, at the end of a (major) segment, the vessel changes its heading, which changes the relative wind and current directions it is facing. We denote the number of subsegments in the vessel route by $M$; $l_i$ is the length of subsegment $i$, and $d_i = d_{i-1} + l_{i}$ with $d_0 = 0$ is the cumulative distance from the beginning of the route. In particular the last breakpoint is the end of the route, $d_M = D$, and we use the two interchangeably from here on.
...
systems that integrate the governing equations of the atmosphere and ocean forward in time from an estimated initial state, and that periodically re-initialise on a fixed cycle.  The source models provide these measurements on native grids of approximately $0.08\degree$ --- between 3 and 5 NM, depending on latitude --- at hourly resolution. The 6 h interval used throughout this study is a sampling choice made for tractability and API budget, not a property of the source data. At each sampling point a forecast was retrieved for the following 168 hours (a week) at hourly lead-time resolution.

\begin{itemize}
\item \textbf{Wind} (10 m speed and direction) --- served by the API's automatic model selection, as the
request does not pin a specific model. The grid coordinates returned along both routes describe an
octahedral reduced-Gaussian grid, with latitude rows $0.0703\degree$ apart and a within-row longitude
spacing of $0.1584\degree$ whose point count varies by row; the resulting cells are approximately 9 km,
equivalent to $0.08\degree$. This is consistent with the ECMWF \emph{Integrated Forecasting System} (IFS)
high-resolution configuration. Re-initialised every 6 h, retrieved to a 168 h horizon.
\item \textbf{Ocean currents} (velocity and direction) --- the M\'et\'eo-France SMOC ocean model, at
$0.08\degree$ resolution, re-initialised once daily, retrieved to a 168 h horizon.
\end{itemize}

For each route, 
two records were collected at every
waypoint on a 6 h sampling cadence over $\sim$170 days and stored in HDF5 with separate tables: the
...
re-plan cadence used later. The wind product refreshes every 6 h and SMOC (currents) every 24 h.
The predicted-weather record confirms this: at a 1 h query cadence, 86\% of consecutive wind values
were identical to the previous hour, and 97\% of consecutive current values.
...
Because the sea conditions change over time, different departure times implies different problem instances.  In our experiment we tested each route with 25 different departure times during the spring of 2026, spaced three days apart.  Each of the $2 \times 25$ route-departure time combinations were tested.
The ETA  (the required length of the voyage) also affect the problem instance. Therefore, an instance in our numerical experiment is determined by the route, departure time and ETA.
With 3 ETA levels corresponding to an average sailing speed of 10, 12, and 14 NM, we obtain a benchmark set consisting of 150  instances. 
...
speed $D/T \pm 3$ kn, that is $V_{\min}=D/T-3$ and $V_{\max}=D/T+3$ --- the band of
Eq.~\eqref{eq:speed-set}, enumerated node-first on the $\delta,\tau$ grid --- and the arrival time $T$
was binding in every run (zero slack) --- so all comparisons are at equal voyage time, isolating the
effect of the setting rather than the schedule's duration. Because the arrival constraint bound in
every run, the zero-speed option that Eq.~\eqref{eq:speed-set} makes available at $d=0$ and $d=D$ was
never exercised: no optimal plan chose to idle.
...
sails a single fixed mean SOG ($D/T$) through the actual time-varying weather, with no optimisation
and no re-planning.
...
Realised fuel is the sum of the committed blocks. The 6 h cadence equals the wind model's NWP cycle (above).
...
run on the Indian Ocean route and twenty-two on the North Atlantic route --- 35 in total --- spanning the full collection window to
date ($\sim$155 days). Rolling-horizon evaluation currently covers the original 19-voyage subset (seven
on the Indian Ocean route, twelve on the North Atlantic route --- the first $\sim$80 days of the same chain); extending it to the full
