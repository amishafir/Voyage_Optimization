...
\end{comment}
% ==== NEW (node-first, continuous interval 𝒱) ====
...
|\mathcal{A}| \;=\; O\!\left(K\,|\mathcal{S}|\right),
\end{equation}
where $\mathcal{A}$ is the induced arc set and $K$ is the number of far-wall grid nodes reachable
within $\mathcal{V}$ from a state (bounded by the speed-band width divided by the grid step). The snap steps are
...
because the node already lies on the grid, no target-versus-realised speed gap arises. Together the
two ranges enumerate exactly the neighbours $\mathcal{A}(d,t)$ of Eq.~\eqref{eq:neighbors} whose
realised speed is admissible ($\bar v\in\mathcal{V}$) --- the same set over which the recursion of
Eq.~\eqref{eq:cost-to-arrive} minimises. A distance line
too close to resolve on the $\tau$-grid --- its snapped arrival collapsing back onto $t$ --- is
skipped, and the vessel glides past it to the next time line. The leg on which a single speed is held is thus of no fixed length:
...
arcs --- the node-first enumeration emits only the $\sim\!8$ distinct reachable far-wall nodes per
