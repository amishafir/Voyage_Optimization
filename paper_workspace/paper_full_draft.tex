...
Given the input above a solution of the deterministic speed control problem is the SOG of the vessel at any given time during the voyage (denoted $v(t)$) so as to minimize the total fuel consumption of the voyage while satisfying the ETA requirement. While under way the SOG of the vessel is seleceted from the band $[V_{\min}, V_{\max}]$ knots ($NM/h$), where the upper bound $V_{\max}$ is the fastest speed the vessel can sustain and the lower bound $V_{\min}>0$ is imposed by the minimum stable engine load and by manoeuvrability. The two ends of the route are exceptions, and they are not symmetric. At the origin ($d=0$) the vessel may idle before departing, so zero speed joins the band. At the destination ($d=D$) it has nowhere further to go: zero is the \emph{only} admissible speed there, and the vessel simply idles until the ETA. Writing $d$ for the along-track distance travelled and $D$ for the total length of the route, the set of admissible speeds is therefore
\begin{equation}
	\label{eq:speed-set}
	\mathcal{V}(d)=
	\begin{cases}
...
Extraction of the actual journeyis then preformed (lines~10--15).  The voyage terminates at $(T,D)$: by
