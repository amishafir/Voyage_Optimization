...
\caption{A state of the dynamic program and its candidate successors in one real interior block of
the Indian Ocean route, voyage~0: distance $d\in[1963.886,1995.719]$ NM and elapsed time
$t\in[120,126]$ h, with $\delta=1$ NM and $\tau=0.1$ h. The horizontal axis is distance,
increasing left to right, and elapsed time increases downward. Panel (a) places the source at
$(t,d)=(123.0\ \mathrm{h},1963.886\ \mathrm{NM})$ on a distance line; panel (b) places it at
$(120.0\ \mathrm{h},1979.719\ \mathrm{NM})$ on a time line. In both panels the same speed cone,
currently generated with $\mathcal{V}=[0,15.119]$ kn, reaches the next distance wall (blue circles)
or the next time wall (green squares); its vertical edge is the $\bar v=0$ wait leg. The markers
show every Eq.~\eqref{eq:neighbors} candidate. Numeric labels and fan lines are thinned
deterministically for legibility, without removing candidate markers.}
