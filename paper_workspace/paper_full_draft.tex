...
Given the input above a solution of the deterministic speed control problem is the SOG of the vessel at any given time during the voyage (denoted $v(t)$) so as to minimize the total fuel consumption of the voyage will satisfying the ETA requirement.    The voyage can be divided into legs during which the course and and the weather conditions are fixed.  Due to the convexity of the FCR function,  in an optimal solution the SOG is fixed during each such leg.  In addition, we assume that the speed of the vessel is seleceted from a discerete set of possibilities, $V$, e.g., $V = \{10.0, 10.1, \ldots,18.5\}$ knots ($NM/h$). Therefore, a solution can be described by a list of such legs and the SOG in each one of them.
...
\item Horizontal \emph{time lines} every six hours and another one at time $T$ (when $T$ is not an integer multiplications of 6) that is at time points $\{6i : i \in \mathbb{N}, 6i < T\} \cup \{T\}$. Between
consecutive time lines sea conditions at each cell are fixed.
\end{itemize}
...
The problem of miniming the total fuel consumption of the voyage can be formulated as the following forward belmen equation:

\begin{equation}
\label{eq:cost-to-arrive}
C^{*}(d,t)=\min_{v \in V}  \begin{cases}  C^*(d-6v, t-6)+6FCR_{i(d)-1}(v,t-6) & \text{if } i(d-6v) +1 = i(d) \vee t \text{ mod } 6 = 0, \\
	C^*(d_{i(d)-1}, 
	t- (d -d_{i(d)-1})/ v)  
	+ (d -d_{i(d)-1})/ v) FCR_{i(d)-1}(v,) & \text{otherwise} 
  \end{cases}
\end{equation}
