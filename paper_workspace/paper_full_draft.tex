...
% ==== OLD (finiteness from a finite speed set V) — superseded below; kept, not rendered ====
\begin{comment}
...
\end{comment}
% ==== NEW (speed drawn from the continuous interval 𝒱; finiteness imposed later by discretisation) ====
The rectangular areas defined by the vertical and horizontal lines are sub-spaces of the distance--time plane in which the optimal speed of the vessel is constant. The only points at which the vessel makes a decision lie on the lines. Since the speed is drawn from the continuous interval $\mathcal{V}=[v_{\min},v_{\max}]$, the reachable points on these lines form a continuum; we render the state set finite in Section~\ref{sec:solve} by discretising the free coordinate onto a regular grid. We define the set of states as
\begin{equation}
	\label{eq:state-space}
	\mathcal{S}=\bigl\{(d,t): (d,t) \text{ is on one of the distance or time lines and it is reachable from point } (0,0) \bigr\}.
\end{equation}
% ==== OLD (fixed speed from finite V) — superseded below; kept, not rendered ====
\begin{comment}
...
\end{comment}
% ==== NEW (fixed speed from the continuous interval 𝒱) ====
where by reachable we mean that the vessel can be at distance--time $(d,t)$ by steaming at some fixed speed from $\mathcal{V}$ in each rectangular sub-space along its voyage through the distance--time plane.
...
\end{equation}
\end{comment}
% ==== NEW (min over the continuous interval 𝒱 — exact; §4.2 approximates by the grid) ====
\begin{equation}
...
\end{equation}
\end{comment}
% ==== NEW (argmin over the continuous interval 𝒱) ====
\begin{equation}
\label{eq:opt-speed}
...
is not tractable.
\end{comment}
% ==== NEW (continuous 𝒱 -> continuum of reachable points; K^lines blow-up of a naive speed grid) ====
...
grid of step $\tau$. We use $\zeta=1$ NM and $\tau=0.1$ h. % ==== OLD (snap merges distinct-speed arrivals; arc bound in |V|) — superseded; kept, not rendered ====
\begin{comment}
...
\end{comment}
% ==== NEW (grid nodes ARE the discrete states/actions; arc bound in K) ====
The grid nodes on each line are then the discrete states, and from a state we enumerate directly the
reachable grid nodes on the next line ahead (Section~\ref{sec:enumerate}) --- so no speed set is ever
introduced. Each distance line carries at most $\lceil T/\tau\rceil$ states and each time line at most
$\lceil L/\zeta\rceil$, giving
\begin{equation}
...
\end{equation}
be the next distance line and next time line strictly ahead. % ==== OLD (iterate over speeds v in V, hold until a line) — superseded; kept, not rendered ====
\begin{comment}
...
the next time line $\overline{t}(t)$, the reachable distance lies between $d+v_{\min}\delta_t$ and
$d+v_{\max}\delta_t$ with $\delta_t=\overline{t}(t)-t$; we take every multiple of $\zeta$. Each
...
\textbf{Input:} distance lines $d_0<\dots<d_M{=}L$, time lines $t_0<\dots<t_\Theta{=}T$, speed band $[v_{\min},v_{\max}]$, snaps $\zeta,\tau$, FCR $\phi$\\
\textbf{Output:} state set $\mathcal{S}$, arc set $\mathcal{A}$\\
$\mathcal{S}\leftarrow\{(0,0)\}$;\quad $\mathcal{A}\leftarrow\emptyset$;\quad queue $Q\leftarrow[(0,0)]$\\
\textbf{while} $Q$ not empty \textbf{do}\\
...
\> \textbf{if} res \textbf{then} \quad{\small$\triangleright$ distance line binds: enumerate arrival \emph{times}}\\
...
\> $d_{\mathrm{cap}}\leftarrow \overline{d}$ \textbf{if} res \textbf{else} $L$ \quad{\small$\triangleright$ time line binds: enumerate arrival \emph{distances} (glide past a too-close line)}\\
\> \textbf{for} $k=\mathrm{round}\!\bigl(\tfrac{d+v_{\min}(\overline{t}-t)}{\zeta}\bigr)\ \textbf{to}\ \mathrm{round}\!\bigl(\tfrac{\min(d+v_{\max}(\overline{t}-t),\,d_{\mathrm{cap}})}{\zeta}\bigr)$ \textbf{do} $C\leftarrow C\cup\{(k\zeta,\,\overline{t})\}$\\
...
node-first on the $\zeta,\tau$ grid --- and the arrival time $T$ was binding in every run (zero slack)
