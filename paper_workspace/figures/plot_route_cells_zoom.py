...
START = (24.75, 52.83)
CELL = 0.5
WINDOW = (54.4, 58.3, 24.9, 27.0)         # lon0, lon1, lat0, lat1 (display)
...
for (la0, lo0), (la1, lo1) in zip(track[:-1], track[1:]):
    mlat = radians((la0 + la1) / 2)
    cum.append(cum[-1] + hypot((la1 - la0) * 60.0, (lo1 - lo0) * 60.0 * cos(mlat)))
track_nm = cum[-1]
SPEED = 3393.2 / 280.0   # kn


def pos_at_nm(target):
    for k in range(len(cum) - 1):
        if cum[k] <= target <= cum[k + 1]:
            f = (target - cum[k]) / (cum[k + 1] - cum[k]) if cum[k + 1] > cum[k] else 0.0
            return (track[k][0] + f * (track[k + 1][0] - track[k][0]),
                    track[k][1] + f * (track[k + 1][1] - track[k][1]))
    return track[-1]


# Build events: cell crossings (space) + heading change + 6 h time blocks (time)
events = [(la, lo, "cell") for (la, lo) in crossings if in_win(la, lo)]
for hla, hlo in HEADING_WPS:
    if in_win(hla, hlo):
        events.append((hla, hlo, "heading"))
tm = 6.0
while SPEED * tm <= track_nm:
    la, lo = pos_at_nm(SPEED * tm)
    if in_win(la, lo):
        events.append((la, lo, "time"))
    tm += 6.0
# drop cell/time crossings within ~9 km of the heading waypoint (they coincide)
...
# Numbered speed-change points (cell = brown, time = blue, heading = navy)
TYPE_COLOR = {"cell": CELLC, "time": TIMEC, "heading": NAVY}
for i, (la, lo, ty) in numbered:
    ax.text(lo, la, str(i), transform=ccrs.PlateCarree(), fontsize=8.5,
            fontweight="bold", color="white", ha="center", va="center", zorder=8,
            bbox=dict(boxstyle="circle,pad=0.26", fc=TYPE_COLOR[ty], ec="white", lw=1.0))
...
ax.legend(handles=[
    ...
    Line2D([0], [0], marker="o", color=TIMEC, lw=0, markersize=8,
           markeredgecolor="white", label="6 h time block (weather, time)"),
