"""
Panel (b) of the state-neighbours pair: the popped state sits ON a
TIME line (a decision epoch), distance interior to a cell. Clean paper
version — the teaching notes live in the joint caption.

Same conventions and band V = [0, v_max] as panel (a).

Run:  python3 plot_state_neighbours_hline.py
Outputs: state_neighbours_hline.pdf, state_neighbours_hline.png
"""
...
d0, t0 = 12.0, 0.0          # the state: ON the time line t = 0, mid-cell
DPREV = 4.0                 # previous distance line (the cell the state is in)
DW, TW = 30.0, 2.4          # next distance wall; next time wall
VMAX = 20.0
TAU, DELTA = 0.2, 2.0
...
# the state's own time line (emphasized) + the next time wall
ax.axhline(0.0, color="#5b6b7a", lw=1.9, ls=(0, (4, 3)), alpha=0.8, zorder=1)
ax.axhline(TW, color=TIMEC, lw=1.0, ls=(0, (4, 3)), zorder=1)
ax.axvline(DPREV, color=NAVY, lw=1.5, alpha=0.4, zorder=1)   # previous distance line
...
ax.annotate(r"$(d,t)$", (d0, t0), xytext=(0, 12), textcoords="offset points",
            fontsize=14, color="#c62828", ha="center")

# short labels
ax.annotate(r"$d_{\mathcal{D}(d)}$", (DW, 0.28), xytext=(7, 0),
            textcoords="offset points", fontsize=13, color=NAVY, va="top")
ax.annotate(r"$t_{\mathcal{T}(t)}$", (DPREV - 2.6, TW), xytext=(0, 9),
