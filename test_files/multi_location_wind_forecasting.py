#!/usr/bin/env python3
"""
Multi-Location Wind Speed and Direction Forecasting Script

Fetches wind weather data from Open-Meteo API for all 13 waypoints
from the ship voyage optimization route (Port A to Port B).

Output:
- One Excel file with summary and per-waypoint sheets
"""
...
OUTPUT_FILENAME = "multi_location_wind_forecast.xlsx"
...
def fetch_wind_data_for_waypoint(client, waypoint):
    """
    Fetch wind weather data for a single waypoint.
    ...
    try:
        ...
        for i, variable in enumerate(CURRENT_VARIABLES):
            if variable == "wind_speed_10m":
                current_data["wind_speed_10m (km/h)"] = current.Variables(i).Value()
            elif variable == "wind_direction_10m":
                current_data["wind_direction_10m (°)"] = current.Variables(i).Value()
...
def fetch_all_waypoints(client, sample_time):
    """
    Fetch wind data for all waypoints.
    ...
    for waypoint in WAYPOINTS:
        try:
            hourly_data, current_data, metadata = fetch_wind_data_for_waypoint(client, waypoint)
            ...
            print(f"  ✓ {waypoint['name']} ({waypoint['lat']}, {waypoint['lon']}): "
                  f"wind={current_data['wind_speed_10m (km/h)']:.1f}km/h @ "
                  f"{current_data['wind_direction_10m (°)']:.0f}°")
...
def wind_speed_to_beaufort(wind_speed_kmh):
    ...
    Based on the Beaufort scale used in the research paper.
    """
    # Convert km/h to m/s
    ...
    with pd.ExcelWriter(excel_path, engine='openpyxl', mode='w') as writer:
        ...
        for run_data in all_runs_data:
            ...
            for wp_data in run_data["waypoints"]:
                ...
                beaufort = wind_speed_to_beaufort(wind_speed)
        ...
        for sample_time in summary_df['sample_time'].unique():
            ...
            for _, row in sample_data.iterrows():
                waypoints_data.append({
                    ...
                    "current_data": {
                        "wind_speed_10m (km/h)": row["wind_speed_10m (km/h)"],
...
def main():
    ...
    print("=" * 70)
    print("Multi-Location Wind Speed and Direction Forecasting")
    print("=" * 70)
    ...
    while run_count < TOTAL_RUNS:
        ...
        print("Fetching wind data for all waypoints...")
