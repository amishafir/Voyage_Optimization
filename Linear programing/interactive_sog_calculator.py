#!/usr/bin/env python3
"""
Interactive SOG Calculator

This script provides an interactive interface for calculating Speed Over Ground (SOG)
using the utility functions and voyage data from Table 8 of the research paper.

User inputs:
- Ship Speed in Still Water (SWS) in knots
- Segment number (1-12)

Output:
- Calculated SOG using paper-exact methodology
- Detailed breakdown of environmental conditions
- Calculation parameters used

Author: Interactive SOG Calculator
Date: 2025
"""

import sys
import math
from typing import Optional

def get_user_input():
    """
    Get user input for SWS and segment number with validation.
    
    Returns:
        tuple: (sws, segment_id) or (None, None) if user wants to exit
    """
    print("🚢 Interactive SOG Calculator")
    print("=" * 50)
    print("Calculate Speed Over Ground using paper-exact methodology")
    print("Based on Table 8 data from the research paper")
    print()
    
    while True:
        try:
            # Get SWS input
            sws_input = input("Enter Ship Speed in Still Water (SWS) in knots [8-15] (or 'q' to quit): ").strip()
            
            if sws_input.lower() in ['q', 'quit', 'exit']:
                return None, None
            
            sws = float(sws_input)
            
            # Validate SWS range
            if sws < 8.0 or sws > 15.0:
                print("⚠️  Warning: SWS should typically be between 8-15 knots for container ships")
                confirm = input("Continue with this speed? (y/n): ").strip().lower()
                if confirm not in ['y', 'yes']:
                    continue
            
            # Get segment input
            segment_input = input("Enter Segment number [1-12]: ").strip()
            segment_id = int(segment_input)
            
            # Validate segment range
            if segment_id < 1 or segment_id > 12:
                print("❌ Error: Segment number must be between 1 and 12")
                continue
            
            return sws, segment_id
            
        except ValueError:
            print("❌ Error: Please enter valid numbers")
            continue
        except KeyboardInterrupt:
            print("\n\n👋 Goodbye!")
            return None, None

def display_segment_info(segment_id: int):
    """
    Display detailed information about the selected segment.
    
    Args:
        segment_id: Segment number (1-12)
    """
    try:
        from voyage_data import get_segment_info
        
        segment_info = get_segment_info(segment_id)
        
        print(f"\n📊 Segment {segment_id} Environmental Conditions:")
        print("=" * 45)
        print(f"Distance: {segment_info['distance_nm']:.1f} nautical miles")
        print(f"Ship Heading (βᵢ): {segment_info['ship_heading_degrees']:.1f}°")
        print(f"Wind Direction (φᵢ): {segment_info['wind_direction_degrees']:.0f}°")
        print(f"Beaufort Scale: {segment_info['beaufort_scale']}")
        print(f"Wave Height: {segment_info['wave_height_m']:.1f} m")
        print(f"Current Speed: {segment_info['current_speed_knots']:.2f} knots")
        print(f"Current Direction (γᵢ): {segment_info['current_direction_degrees']:.0f}°")
        
        # Calculate weather direction angle
        weather_angle = abs(segment_info['wind_direction_degrees'] - segment_info['ship_heading_degrees'])
        if weather_angle > 180:
            weather_angle = 360 - weather_angle
        
        print(f"Weather Direction Angle: {weather_angle:.1f}°")
        
        # Determine sea conditions
        beaufort = segment_info['beaufort_scale']
        if beaufort <= 2:
            sea_condition = "Calm"
        elif beaufort <= 4:
            sea_condition = "Moderate"
        elif beaufort <= 6:
            sea_condition = "Rough"
        else:
            sea_condition = "Very Rough"
        
        print(f"Sea Condition: {sea_condition}")
        
    except ImportError:
        print("⚠️  Warning: voyage_data module not found")
    except Exception as e:
        print(f"⚠️  Warning: Could not load segment info: {e}")

def display_detailed_calculation_breakdown(sws: float, segment_data: dict):
    """
    Display comprehensive step-by-step calculation breakdown with all formulas and parameters.
    
    Args:
        sws: Ship speed in still water (knots)
        segment_data: Segment data dictionary
    """
    try:
        from utility_functions import (
            calculate_speed_over_ground,
            calculate_weather_direction_angle,
            calculate_froude_number,
            calculate_direction_reduction_coefficient,
            calculate_speed_reduction_coefficient,
            calculate_ship_form_coefficient,
            calculate_speed_loss_percentage,
            calculate_weather_corrected_speed,
            calculate_sog_vector_synthesis,
            KNOTS_TO_MS, MS_TO_KNOTS
        )
        from voyage_data import SHIP_PARAMETERS
        
        print(f"\n🔬 COMPREHENSIVE SOG CALCULATION BREAKDOWN")
        print("=" * 80)
        print("Following the exact methodology from the research paper")
        print("'Ship Speed Optimization Considering Ocean Currents to Enhance")
        print("Environmental Sustainability in Maritime Shipping'")
        print("=" * 80)
        
        # ===============================
        # STEP 1: INPUT PARAMETERS
        # ===============================
        print(f"\n📋 STEP 1: INPUT PARAMETERS")
        print("-" * 40)
        print(f"Ship Speed in Still Water (Vsw):    {sws:.3f} knots")
        print(f"Ocean Current Speed (Vc):           {segment_data['ocean_current']:.3f} knots")
        print(f"Current Direction (γ):              {math.degrees(segment_data['current_direction']):.1f}° = {segment_data['current_direction']:.4f} rad")
        print(f"Ship Heading (α/β):                 {math.degrees(segment_data['ship_heading']):.1f}° = {segment_data['ship_heading']:.4f} rad")
        print(f"Wind Direction (φ):                 {math.degrees(segment_data['wind_direction']):.1f}° = {segment_data['wind_direction']:.4f} rad")
        print(f"Beaufort Scale (BN):                {segment_data['beaufort_scale']}")
        print(f"Wave Height (H):                    {segment_data['wave_height']:.1f} m")
        print(f"Segment Distance:                   {segment_data['distance']:.1f} nm")
        
        print(f"\nShip Parameters:")
        for param, value in SHIP_PARAMETERS.items():
            if isinstance(value, float):
                print(f"  {param.replace('_', ' ').title():.<25} {value:.1f}")
            else:
                print(f"  {param.replace('_', ' ').title():.<25} {value}")
        
        # ===============================
        # STEP 2: WEATHER DIRECTION ANGLE
        # ===============================
        print(f"\n🌊 STEP 2: WEATHER DIRECTION ANGLE CALCULATION")
        print("-" * 50)
        print("Formula: θ = |φ - α|  (Paper Equation 9)")
        print("where θ = weather direction angle")
        print("      φ = wind direction")
        print("      α = ship heading")
        
        weather_angle_rad = calculate_weather_direction_angle(
            segment_data['wind_direction'], 
            segment_data['ship_heading']
        )
        weather_angle_deg = math.degrees(weather_angle_rad)
        
        print(f"\nCalculation:")
        print(f"θ = |{math.degrees(segment_data['wind_direction']):.1f}° - {math.degrees(segment_data['ship_heading']):.1f}°|")
        print(f"θ = |{math.degrees(segment_data['wind_direction']) - math.degrees(segment_data['ship_heading']):.1f}°|")
        print(f"θ = {weather_angle_deg:.1f}° = {weather_angle_rad:.4f} rad")
        
        # ===============================
        # STEP 3: FROUDE NUMBER
        # ===============================
        print(f"\n⚓ STEP 3: FROUDE NUMBER CALCULATION")
        print("-" * 40)
        print("Formula: Fn = V / √(g × L)")
        print("where Fn = Froude number (dimensionless)")
        print("      V  = ship speed (m/s)")
        print("      g  = gravitational acceleration (9.81 m/s²)")
        print("      L  = ship length (m)")
        
        ship_speed_ms = sws * KNOTS_TO_MS
        froude_number = calculate_froude_number(ship_speed_ms, SHIP_PARAMETERS['length'])
        
        print(f"\nCalculation:")
        print(f"V = {sws:.3f} knots × {KNOTS_TO_MS:.4f} = {ship_speed_ms:.3f} m/s")
        print(f"Fn = {ship_speed_ms:.3f} / √(9.81 × {SHIP_PARAMETERS['length']:.1f})")
        print(f"Fn = {ship_speed_ms:.3f} / √({9.81 * SHIP_PARAMETERS['length']:.1f})")
        print(f"Fn = {ship_speed_ms:.3f} / {math.sqrt(9.81 * SHIP_PARAMETERS['length']):.3f}")
        print(f"Fn = {froude_number:.6f}")
        
        # ===============================
        # STEP 4: DIRECTION REDUCTION COEFFICIENT
        # ===============================
        print(f"\n🧭 STEP 4: DIRECTION REDUCTION COEFFICIENT (Cβ)")
        print("-" * 50)
        print("Based on Paper Table 2 - Weather direction relative to ship's bow")
        
        c_beta = calculate_direction_reduction_coefficient(weather_angle_deg, segment_data['beaufort_scale'])
        
        # Determine which formula applies
        if 0 <= weather_angle_deg <= 30:
            sea_type = "Head sea (0-30°)"
            formula = "Cβ = 2.0"
        elif 30 < weather_angle_deg <= 60:
            sea_type = "Bow sea (30-60°)"
            formula = f"Cβ = 1.7 - 0.03 × (BN - 4)²"
        elif 60 < weather_angle_deg <= 150:
            sea_type = "Beam sea (60-150°)"
            formula = f"Cβ = 0.9 - 0.06 × (BN - 6)²"
        else:
            sea_type = "Following sea (150-180°)"
            formula = f"Cβ = 0.4 - 0.03 × (BN - 8)²"
        
        print(f"\nSea Condition: {sea_type}")
        print(f"Formula: {formula}")
        print(f"where BN = Beaufort Number = {segment_data['beaufort_scale']}")
        
        if 30 < weather_angle_deg <= 60:
            bn_term = segment_data['beaufort_scale'] - 4
            print(f"\nCalculation:")
            print(f"Cβ = 1.7 - 0.03 × ({segment_data['beaufort_scale']} - 4)²")
            print(f"Cβ = 1.7 - 0.03 × ({bn_term})²")
            print(f"Cβ = 1.7 - 0.03 × {bn_term**2}")
            print(f"Cβ = 1.7 - {0.03 * bn_term**2:.3f}")
            print(f"Cβ = {c_beta:.3f}")
        elif 60 < weather_angle_deg <= 150:
            bn_term = segment_data['beaufort_scale'] - 6
            print(f"\nCalculation:")
            print(f"Cβ = 0.9 - 0.06 × ({segment_data['beaufort_scale']} - 6)²")
            print(f"Cβ = 0.9 - 0.06 × ({bn_term})²")
            print(f"Cβ = 0.9 - 0.06 × {bn_term**2}")
            print(f"Cβ = 0.9 - {0.06 * bn_term**2:.3f}")
            print(f"Cβ = {c_beta:.3f}")
        elif weather_angle_deg > 150:
            bn_term = segment_data['beaufort_scale'] - 8
            print(f"\nCalculation:")
            print(f"Cβ = 0.4 - 0.03 × ({segment_data['beaufort_scale']} - 8)²")
            print(f"Cβ = 0.4 - 0.03 × ({bn_term})²")
            print(f"Cβ = 0.4 - 0.03 × {bn_term**2}")
            print(f"Cβ = 0.4 - {0.03 * bn_term**2:.3f}")
            print(f"Cβ = {c_beta:.3f}")
        else:
            print(f"\nCalculation:")
            print(f"Cβ = {c_beta:.3f} (constant for head sea)")
        
        # ===============================
        # STEP 5: SPEED REDUCTION COEFFICIENT
        # ===============================
        print(f"\n🚀 STEP 5: SPEED REDUCTION COEFFICIENT (CU)")
        print("-" * 50)
        print("Based on Paper Table 3 - Froude number and block coefficient")
        
        cb = SHIP_PARAMETERS['block_coefficient']
        c_u = calculate_speed_reduction_coefficient(froude_number, cb, 'normal')
        
        # Determine which formula applies
        if cb <= 0.55:
            formula = "CU = 1.7 - 1.4×Fn - 7.4×Fn²"
        elif cb <= 0.60:
            formula = "CU = 2.2 - 2.5×Fn - 9.7×Fn²"
        elif cb <= 0.65:
            formula = "CU = 2.6 - 3.7×Fn - 11.6×Fn²"
        elif cb <= 0.70:
            formula = "CU = 3.1 - 5.3×Fn - 12.4×Fn²"
        elif cb <= 0.75:
            formula = "CU = 2.4 - 10.6×Fn - 9.5×Fn² (normal loading)"
        elif cb <= 0.80:
            formula = "CU = 2.6 - 13.1×Fn - 15.1×Fn² (normal loading)"
        else:
            formula = "CU = 3.1 - 18.7×Fn + 28.0×Fn² (normal loading)"
        
        print(f"\nBlock Coefficient (Cb): {cb:.3f}")
        print(f"Loading Condition: Normal")
        print(f"Formula: {formula}")
        
        print(f"\nCalculation:")
        print(f"Fn = {froude_number:.6f}")
        print(f"Fn² = {froude_number**2:.6f}")
        
        if cb <= 0.55:
            term1 = 1.7
            term2 = -1.4 * froude_number
            term3 = -7.4 * froude_number**2
        elif cb <= 0.60:
            term1 = 2.2
            term2 = -2.5 * froude_number
            term3 = -9.7 * froude_number**2
        elif cb <= 0.65:
            term1 = 2.6
            term2 = -3.7 * froude_number
            term3 = -11.6 * froude_number**2
        elif cb <= 0.70:
            term1 = 3.1
            term2 = -5.3 * froude_number
            term3 = -12.4 * froude_number**2
        elif cb <= 0.75:
            term1 = 2.4
            term2 = -10.6 * froude_number
            term3 = -9.5 * froude_number**2
        elif cb <= 0.80:
            term1 = 2.6
            term2 = -13.1 * froude_number
            term3 = -15.1 * froude_number**2
        else:
            term1 = 3.1
            term2 = -18.7 * froude_number
            term3 = 28.0 * froude_number**2
        
        print(f"CU = {term1:.1f} + ({term2:.6f}) + ({term3:.6f})")
        print(f"CU = {term1 + term2 + term3:.6f}")
        print(f"CU = {c_u:.6f} (with engineering bounds applied)")
        
        # ===============================
        # STEP 6: SHIP FORM COEFFICIENT
        # ===============================
        print(f"\n🏗️ STEP 6: SHIP FORM COEFFICIENT (CForm)")
        print("-" * 45)
        print("Based on Paper Table 4 - Beaufort scale and displacement")
        print("Formula: CForm = 0.5×BN + BN^6.5/(22×∇^(2/3))  (normal loading)")
        print("where BN = Beaufort number")
        print("      ∇  = displacement volume (m³)")
        
        # Calculate displacement volume from tonnage
        displacement_volume = SHIP_PARAMETERS['displacement'] * 1000 / 1025  # tonnes to m³ (seawater density)
        c_form = calculate_ship_form_coefficient(segment_data['beaufort_scale'], displacement_volume, 'normal')
        
        print(f"\nCalculation:")
        print(f"Displacement = {SHIP_PARAMETERS['displacement']:.0f} tonnes")
        print(f"∇ = {SHIP_PARAMETERS['displacement']:.0f} × 1000 / 1025 = {displacement_volume:.1f} m³")
        print(f"∇^(2/3) = {displacement_volume:.1f}^(2/3) = {displacement_volume**(2/3):.1f}")
        print(f"BN = {segment_data['beaufort_scale']}")
        print(f"BN^6.5 = {segment_data['beaufort_scale']}^6.5 = {segment_data['beaufort_scale']**6.5:.1f}")
        
        term1 = 0.5 * segment_data['beaufort_scale']
        term2 = (segment_data['beaufort_scale']**6.5) / (22 * displacement_volume**(2/3))
        
        print(f"\nCForm = 0.5×{segment_data['beaufort_scale']} + {segment_data['beaufort_scale']**6.5:.1f}/(22×{displacement_volume**(2/3):.1f})")
        print(f"CForm = {term1:.3f} + {term2:.6f}")
        print(f"CForm = {c_form:.6f}")
        
        # ===============================
        # STEP 7: SPEED LOSS PERCENTAGE
        # ===============================
        print(f"\n📉 STEP 7: SPEED LOSS PERCENTAGE")
        print("-" * 35)
        print("Formula: ΔV/Vsw × 100% = Cβ × CU × CForm  (Paper Equation 7)")
        
        speed_loss_percent = calculate_speed_loss_percentage(c_beta, c_u, c_form)
        
        print(f"\nCalculation:")
        print(f"ΔV/Vsw × 100% = {c_beta:.6f} × {c_u:.6f} × {c_form:.6f}")
        print(f"ΔV/Vsw × 100% = {c_beta * c_u * c_form:.6f}")
        print(f"Speed Loss = {speed_loss_percent:.3f}%")
        
        # ===============================
        # STEP 8: WEATHER-CORRECTED SPEED
        # ===============================
        print(f"\n🌤️ STEP 8: WEATHER-CORRECTED SPEED")
        print("-" * 40)
        print("Formula: Vw = Vsw × (1 - ΔV/Vsw)  (Paper Equation 8)")
        
        weather_corrected_speed = calculate_weather_corrected_speed(sws, speed_loss_percent)
        
        print(f"\nCalculation:")
        print(f"Vw = {sws:.3f} × (1 - {speed_loss_percent:.6f}/100)")
        print(f"Vw = {sws:.3f} × (1 - {speed_loss_percent/100:.6f})")
        print(f"Vw = {sws:.3f} × {1 - speed_loss_percent/100:.6f}")
        print(f"Vw = {weather_corrected_speed:.6f} knots")
        
        # ===============================
        # STEP 9: VECTOR SYNTHESIS FOR SOG
        # ===============================
        print(f"\n🧭 STEP 9: SPEED OVER GROUND (SOG) - VECTOR SYNTHESIS")
        print("-" * 55)
        print("Formulas: (Paper Equations 14-16)")
        print("Vx = Vw × sin(α) + Vc × sin(γ)  (Equation 14)")
        print("Vy = Vw × cos(α) + Vc × cos(γ)  (Equation 15)")
        print("Vg = √(Vx² + Vy²)              (Equation 16)")
        print("\nwhere: Vw = weather-corrected speed")
        print("       α  = ship heading")
        print("       Vc = ocean current speed")
        print("       γ  = current direction")
        
        # Calculate velocity components
        vw_x = weather_corrected_speed * math.sin(segment_data['ship_heading'])
        vw_y = weather_corrected_speed * math.cos(segment_data['ship_heading'])
        vc_x = segment_data['ocean_current'] * math.sin(segment_data['current_direction'])
        vc_y = segment_data['ocean_current'] * math.cos(segment_data['current_direction'])
        vg_x = vw_x + vc_x
        vg_y = vw_y + vc_y
        
        sog = calculate_sog_vector_synthesis(
            weather_corrected_speed, 
            segment_data['ship_heading'],
            segment_data['ocean_current'], 
            segment_data['current_direction']
        )
        
        print(f"\nStep 9a: Ship velocity components")
        print(f"Vw_x = {weather_corrected_speed:.6f} × sin({math.degrees(segment_data['ship_heading']):.1f}°)")
        print(f"Vw_x = {weather_corrected_speed:.6f} × {math.sin(segment_data['ship_heading']):.6f} = {vw_x:.6f}")
        print(f"Vw_y = {weather_corrected_speed:.6f} × cos({math.degrees(segment_data['ship_heading']):.1f}°)")
        print(f"Vw_y = {weather_corrected_speed:.6f} × {math.cos(segment_data['ship_heading']):.6f} = {vw_y:.6f}")
        
        print(f"\nStep 9b: Current velocity components")
        print(f"Vc_x = {segment_data['ocean_current']:.6f} × sin({math.degrees(segment_data['current_direction']):.1f}°)")
        print(f"Vc_x = {segment_data['ocean_current']:.6f} × {math.sin(segment_data['current_direction']):.6f} = {vc_x:.6f}")
        print(f"Vc_y = {segment_data['ocean_current']:.6f} × cos({math.degrees(segment_data['current_direction']):.1f}°)")
        print(f"Vc_y = {segment_data['ocean_current']:.6f} × {math.cos(segment_data['current_direction']):.6f} = {vc_y:.6f}")
        
        print(f"\nStep 9c: Total velocity components")
        print(f"Vg_x = Vw_x + Vc_x = {vw_x:.6f} + {vc_x:.6f} = {vg_x:.6f}")
        print(f"Vg_y = Vw_y + Vc_y = {vw_y:.6f} + {vc_y:.6f} = {vg_y:.6f}")
        
        print(f"\nStep 9d: Final SOG calculation")
        print(f"Vg = √(Vg_x² + Vg_y²)")
        print(f"Vg = √(({vg_x:.6f})² + ({vg_y:.6f})²)")
        print(f"Vg = √({vg_x**2:.6f} + {vg_y**2:.6f})")
        print(f"Vg = √({vg_x**2 + vg_y**2:.6f})")
        
        # Calculate raw SOG before post-processing
        sog_raw = math.sqrt(vg_x**2 + vg_y**2)
        print(f"Vg (raw) = {sog_raw:.6f} knots")
        
        # ===============================
        # STEP 10: POST-PROCESSING FOR SEVERE WEATHER
        # ===============================
        if segment_data['beaufort_scale'] >= 5:
            sog_final = sog_raw * 0.965
            print(f"\n⚠️ STEP 10: POST-PROCESSING FOR SEVERE WEATHER CONDITIONS")
            print("-" * 58)
            print(f"Beaufort Scale: {segment_data['beaufort_scale']} (≥5 - Rough/Very Rough conditions)")
            print("Additional 3.5% speed reduction applied for severe weather")
            print("Formula: SOG_final = SOG_raw × 0.965")
            print(f"\nCalculation:")
            print(f"SOG_final = {sog_raw:.6f} × 0.965")
            print(f"SOG_final = {sog_final:.6f} knots")
            print(f"Additional reduction: {sog_raw - sog_final:.6f} knots ({(sog_raw - sog_final)/sog_raw * 100:.1f}%)")
            # Update sog to the final processed value for consistency
            sog = sog_final
        else:
            print(f"\n✅ STEP 10: NO ADDITIONAL POST-PROCESSING")
            print("-" * 42)
            print(f"Beaufort Scale: {segment_data['beaufort_scale']} (<5 - Moderate conditions)")
            print("No additional weather correction applied")
            print(f"Final SOG = {sog:.6f} knots")
        
        # ===============================
        # STEP 11: SUMMARY
        # ===============================
        print(f"\n🎯 FINAL RESULTS SUMMARY")
        print("=" * 40)
        print(f"Ship Speed in Still Water (Vsw):    {sws:.3f} knots")
        print(f"Weather-Corrected Speed (Vw):       {weather_corrected_speed:.3f} knots")
        print(f"Speed Over Ground (SOG):             {sog:.3f} knots")
        print(f"Speed Difference (SOG - Vsw):       {sog - sws:+.3f} knots")
        print(f"Speed Change Percentage:             {(sog - sws)/sws * 100:+.1f}%")
        
        print(f"\nKey Coefficients:")
        print(f"Direction Reduction (Cβ):           {c_beta:.6f}")
        print(f"Speed Reduction (CU):                {c_u:.6f}")
        print(f"Ship Form (CForm):                   {c_form:.6f}")
        print(f"Speed Loss:                          {speed_loss_percent:.3f}%")
        
        print(f"\nTravel Time Analysis:")
        travel_time = segment_data['distance'] / sog
        travel_time_vsw = segment_data['distance'] / sws
        time_difference = travel_time - travel_time_vsw
        
        print(f"Distance:                            {segment_data['distance']:.1f} nm")
        print(f"Travel Time at SOG:                  {travel_time:.2f} hours")
        print(f"Travel Time at SWS:                  {travel_time_vsw:.2f} hours")
        print(f"Time Difference:                     {time_difference:+.2f} hours ({time_difference*60:+.0f} min)")
        
        print(f"\n✅ Complete calculation breakdown finished!")
        print("=" * 80)
        
    except ImportError as e:
        print(f"⚠️  Warning: Could not import required functions: {e}")
    except Exception as e:
        print(f"⚠️  Warning: Error in detailed breakdown: {e}")
        import traceback
        traceback.print_exc()

def calculate_and_display_sog(sws: float, segment_id: int):
    """
    Calculate SOG and display detailed results.
    
    Args:
        sws: Ship speed in still water (knots)
        segment_id: Segment number (1-12)
    """
    try:
        from utility_functions import calculate_sog_from_voyage_data
        from voyage_data import get_segment_for_utility_functions, SHIP_PARAMETERS
        
        print(f"\n🧮 SOG Calculation for Segment {segment_id}")
        print("=" * 45)
        
        # Calculate SOG using the utility function
        sog = calculate_sog_from_voyage_data(segment_id, sws)
        
        # Get segment data for detailed display
        segment_data = get_segment_for_utility_functions(segment_id, sws)
        
        # Calculate some additional metrics
        speed_difference = sog - sws
        speed_change_percent = (speed_difference / sws) * 100
        
        print(f"Input SWS: {sws:.2f} knots")
        print(f"Calculated SOG: {sog:.3f} knots")
        print(f"Speed Difference: {speed_difference:+.3f} knots")
        print(f"Speed Change: {speed_change_percent:+.1f}%")
        
        # Calculate travel time for this segment
        distance = segment_data['distance']
        travel_time = distance / sog
        
        print(f"\n⏱️  Travel Time Calculation:")
        print(f"Segment Distance: {distance:.1f} nm")
        print(f"Travel Time: {travel_time:.2f} hours")
        print(f"Travel Time: {travel_time*60:.0f} minutes")
        
        # Environmental impact
        if speed_difference > 0:
            impact = "Favorable (current and/or wind assisting)"
        elif speed_difference < -0.5:
            impact = "Adverse (significant weather/current penalty)"
        else:
            impact = "Neutral (minimal environmental impact)"
        
        print(f"\n🌊 Environmental Impact: {impact}")
        
        # Always show detailed calculation breakdown
        display_detailed_calculation_breakdown(sws, segment_data)
        
        return sog
        
    except ImportError as e:
        print(f"❌ Error: Required modules not found: {e}")
        return None
    except Exception as e:
        print(f"❌ Error calculating SOG: {e}")
        return None

def main():
    """
    Main function to run the interactive SOG calculator.
    """
    print("🚢 Welcome to the Interactive SOG Calculator!")
    print("Using paper-exact methodology from utility_functions.py")
    print("Environmental data from Table 8 of the research paper")
    print()
    
    while True:
        # Get user input
        sws, segment_id = get_user_input()
        
        if sws is None or segment_id is None:  # User wants to exit
            print("\n👋 Thank you for using the SOG Calculator!")
            break
        
        # Display segment information
        display_segment_info(segment_id)
        
        # Calculate and display SOG
        sog = calculate_and_display_sog(sws, segment_id)
        
        if sog is not None:
            print(f"\n✅ Calculation Complete!")
            print(f"📊 Summary: SWS {sws:.2f} knots → SOG {sog:.3f} knots (Segment {segment_id})")
        
        # Ask if user wants to continue
        print("\n" + "=" * 50)
        continue_calc = input("Calculate another SOG? (y/n): ").strip().lower()
        if continue_calc not in ['y', 'yes']:
            print("\n👋 Thank you for using the SOG Calculator!")
            break
        
        print("\n" + "=" * 50)

def quick_calculation(sws: float, segment_id: int):
    """
    Quick calculation function for scripting use.
    
    Args:
        sws: Ship speed in still water (knots)
        segment_id: Segment number (1-12)
    
    Returns:
        float: Calculated SOG or None if error
    """
    try:
        from utility_functions import calculate_sog_from_voyage_data
        return calculate_sog_from_voyage_data(segment_id, sws)
    except:
        return None

if __name__ == "__main__":
    # Check if command line arguments are provided for quick calculation
    if len(sys.argv) == 3:
        try:
            sws = float(sys.argv[1])
            segment_id = int(sys.argv[2])
            
            if 1 <= segment_id <= 12:
                sog = quick_calculation(sws, segment_id)
                if sog is not None:
                    print(f"SOG Calculation: SWS {sws:.2f} knots → SOG {sog:.3f} knots (Segment {segment_id})")
                else:
                    print("Error: Could not calculate SOG")
            else:
                print("Error: Segment must be between 1 and 12")
        except ValueError:
            print("Error: Invalid input arguments")
            print("Usage: python interactive_sog_calculator.py <SWS> <segment_id>")
    else:
        # Run interactive mode
        main() 