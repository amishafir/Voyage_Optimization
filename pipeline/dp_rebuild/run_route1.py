...
def main():
    ...
    frame = frame_from_route(route, voyage, waypoints, base_sample_hour=sample_hour)
    ...
    print(f"Weather mode: per-block actual (Mode C)  "
          f"base_sample_hour={sample_hour}, blocks {sample_hour}..{sample_hour + int(frame.cfg.eta_h) - 6} step 6")

    # ---- Build atomic graph -----------------------------------------------
    # override_sample_hour=None → SR/Luo DP read actual_weather per-block at
    # sample_hour = base_sample_hour + 6·block_index (Mode C, oracle planning).
    ...
    res = simulate_steady_voyage(
        ...
        base_sample_hour=sample_hour,  # Mode C: per-block actual
    )
