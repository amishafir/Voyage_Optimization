...
The relationship between the engine's still water speed $V_s$ and the achieved speed over ground $V_g$ is governed by the eight-step speed correction model of [CITE: Yang2020]. The model computes a percentage speed loss from wind-induced resistance — carried through the Beaufort number — and from ocean currents, through empirical coefficient lookups that depend on the Froude number, Beaufort number, relative wind angle, block coefficient, and displacement volume. The weather-corrected speed through water is then combined with ocean current velocity via two-dimensional vector synthesis to yield the final SOG. The complete model maps any combination of SWS $V_s$ and environmental conditions $(BN, \theta, V_c, \gamma)$ to a unique SOG $V_g$. This mapping is monotonically increasing in $V_s$: higher engine speed always yields higher ground speed, though the relationship is nonlinear and weather-dependent. A detailed derivation with all coefficient tables is provided in [CITE: Yang2020].
