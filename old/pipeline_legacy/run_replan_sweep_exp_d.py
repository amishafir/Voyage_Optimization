#!/usr/bin/env python3
"""Run replan frequency sweep on experiment_d and analyze effective new information.
...
import yaml
from shared.hdf5_io import read_predicted, get_completed_runs
...
# Infer collection hours from HDF5 (config has hours=0 for indefinite)
completed = get_completed_runs(hdf5_path)
max_sample_hour = max(completed) if completed else 0
...
def run_sweep():
    ...
    print("=" * 80)
    print(f"REPLAN FREQUENCY SWEEP — Experiment D (389 WP, {max_sample_hour}h collection)")
    print("=" * 80)

    for freq in FREQUENCIES:
        cfg = copy.deepcopy(config)
        cfg["dynamic_rh"]["replan_frequency_hours"] = freq
        approach_name = f"dynamic_rh_replan_{freq}h_exp_d"
        ...
        if not decision_hours:
            decision_hours = list(range(0, max_sample_hour + 1, freq))
    ...
    baseline_fuel = None
    for r in results:
        freq_str = r["approach"].replace("_exp_d", "").split("_")[-1]
