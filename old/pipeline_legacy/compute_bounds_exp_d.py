#!/usr/bin/env python3
"""
Compute fuel upper and lower bounds for experiment_d (389 nodes, ~163h).
...
# ── AVERAGE BOUND: constant SOG ──
print("=" * 60)
print("AVERAGE BOUND — Constant SOG (total distance / ETA)")
print("=" * 60)

avg_sog = total_dist / eta
avg_sws = avg_sog  # In calm water, SWS ≈ SOG
avg_fcr = calculate_fuel_consumption_rate(avg_sws)
avg_fuel = avg_fcr * eta

print(f"  Avg SOG = {avg_sog:.2f} kn")
print(f"  Fuel = {avg_fuel:.2f} mt (calm water, constant speed)")
print()
...
weather_tax = fuel_lb - avg_fuel
print(f"  Upper bound:   {fuel_ub:.2f} mt  (SWS={max_speed} kn, arrives {eta - time_ub:.1f}h early)")
print(f"  Optimal bound: {fuel_lb:.2f} mt  (optimal per-node SWS, actual weather)")
print(f"  Average bound: {avg_fuel:.2f} mt  (constant SOG={avg_sog:.2f} kn, calm water)")
print(f"  Span:          {span:.2f} mt  (upper - optimal)")
print(f"  Weather tax:   {weather_tax:.2f} mt  (optimal - average)")
