#!/usr/bin/env python3
"""
Multi-Location Wind and Wave Forecasting Script - 170 WAYPOINTS VERSION

Fetches wind and marine weather data from Open-Meteo API for ~170 evenly spaced
waypoints (every 20th point from the 3,388 interpolated waypoints).

This version stays within Open-Meteo free tier daily API limits.

Output:
- voyage_nodes_170wp_weather.pickle: List of Node objects with weather data
"""
...
# Waypoint sampling - every Nth waypoint from the 3,388
WAYPOINT_SAMPLE_INTERVAL = 20  # Take every 20th waypoint = ~170 waypoints

# Schedule Configuration
INTERVAL_MINUTES = 60  # 1 hour
TOTAL_RUNS = 36  # 36 hours (1.5 days)
DURATION_HOURS = 36

# Output Configuration
SCRIPT_DIR = Path(__file__).parent.absolute()
WAYPOINTS_FILE = SCRIPT_DIR / "waypoints_interpolated.txt"
OUTPUT_FILENAME = "voyage_nodes_170wp_weather.pickle"
...
def load_waypoints_from_file(filepath, sample_interval=1):
    """Parse waypoints_interpolated.txt and return list of waypoint dicts.

    Args:
        filepath: Path to waypoints file
        sample_interval: Take every Nth waypoint (1 = all, 20 = every 20th)
    """
    all_waypoints = []
    ...
    # Sample waypoints - always include first and last
    if sample_interval > 1:
        sampled = []
        for i, wp in enumerate(all_waypoints):
            # Include if: first, last, every Nth, or original waypoint
            if i == 0 or i == len(all_waypoints) - 1 or i % sample_interval == 0 or wp["is_original"]:
                sampled.append(wp)
        return sampled
...
def setup_api_client():
    ...
    cache_session = requests_cache.CachedSession('.cache_170wp', expire_after=3600)
...
def main():
    ...
    print("=" * 70)
    print("Multi-Location Forecast - 170 WAYPOINTS VERSION")
    print("=" * 70)
    print(f"Output file: {OUTPUT_PATH}")
    print(f"Schedule: Every {INTERVAL_MINUTES} minutes for {DURATION_HOURS} hours ({TOTAL_RUNS} runs)")
    print(f"Sampling: Every {WAYPOINT_SAMPLE_INTERVAL}th waypoint (+ all original waypoints)")
    print("=" * 70)
    print()

    # Load waypoints from file (sampled)
    print(f"Loading waypoints from: {WAYPOINTS_FILE}")
    waypoints = load_waypoints_from_file(WAYPOINTS_FILE, sample_interval=WAYPOINT_SAMPLE_INTERVAL)
    print(f"Loaded {len(waypoints)} waypoints (sampled from 3,388)")
    print()

    # API usage estimate
    calls_per_run = len(waypoints) * 2
    calls_per_day = calls_per_run * 24
    print(f"API calls per run: {calls_per_run}")
    print(f"API calls per day: {calls_per_day} (limit: ~10,000)")
    print()
        ...
        for i, (wp, node) in enumerate(zip(waypoints, nodes)):
            ...
            if error:
                ...
                if failed <= 5:  # Only show first 5 errors
                    print(f"  X [{i+1}/{len(waypoints)}] {wp['name']}: {error}")
            else:
            ...
            # Progress update every 20 waypoints
            if (i + 1) % 20 == 0:
