...
**T21 — The picture + full notation key (foundation for reading §4).** `[status: verified; = docs/state_space_evolution.html]` `[added 2026-07-26]`
- **The picture:** everything lives on the **time–distance plane** — distance `d` across (0 → L), time `t` down (0 → T). The voyage is a line from top-left `(0,0)` to the right edge; **steepness = speed** (shallow = fast, steep = slow). Vertical lines = cell/heading boundaries (space); horizontal lines = 6 h blocks (time). Decisions happen only **on** the lines.
- **Every symbol, in plain words:** `d,t` = a point ("d along the route, t hours in") · `d₀…d_M` = the distance lines (M subsegments) · `t₀…t_Θ` = the time lines (Θ blocks) · `δ = 1 NM` = distance step of the grid (on time lines) · `τ = 0.1 h` = time step (on distance lines) · `𝒮` = the state set: every grid dot where a decision can be made · `𝒟(d)/𝒯(t)` = "the next wall ahead" in space/time · `𝒜(d,t)` = the neighbours: states reachable in one straight leg · `𝒱 = [v_min,v_max]` = the allowed speed band · `v̄ = Δd/Δt` = the **realised** speed of a leg (read off, not chosen) · `φ(d,t;v)` = fuel-burn rate in the cell at (d,t) at speed v · `C*(d,t)` = cheapest fuel to arrive · `V*(d,t)` = the recorded optimal speed · `(d̃,t̃)` = a candidate **predecessor** · `⟨x⟩_δ` = snap: round x to the nearest multiple of δ · `d̄/t̄` = shorthand for d_{𝒟(d)}/t_{𝒯(t)} · `κ` = landing points reachable from one state (≈ band width ÷ step ≈ 8) · `|𝒮|,|𝒜|` = search size.

**T22 — Acts 1–2: the explicit grid 𝒮 and the completed two-case 𝒜(d,t), symbol by symbol.** `[status: verified; Tal c0dc083 + a8ae392]` `[added 2026-07-26]`
- **𝒮 (Eq. 4) = two grid families ∪ the origin.** First family: dots **on the time lines** — start at each distance line `d_i` and step back by multiples of δ (`(d_i − nδ, t_j)`). Second: dots **on the distance lines** — step back from each time line by multiples of τ (`(d_i, t_j − nτ)`). The floor terms `⌊(d_i−d_{i−1})/δ⌋`, `⌊(t_j−t_{j−1})/τ⌋` count how many whole steps fit in the cell/block. **Plain words:** "list every grid dot sitting on a ruler line, plus the start — that finite list is every place a speed decision might be made."
- **𝒟/𝒯:** just names for "the first line strictly past you" (argmin over indices), so 𝒜 can be written compactly.
- **𝒜(d,t) (Eq. 5) = union of two mirror cases.** **Time case (Tal):** neighbours on the next *distance* line `d_{𝒟(d)}`, arrival time `t′ = t_{𝒯(t)} − nτ` — "sail to the next cell boundary; any τ-slot in this block is a landing spot." **Distance case (added):** neighbours on the next *time* line `t_{𝒯(t)}`, arrival distance `d′ = d_{𝒟(d)} − nδ` — "the 6 h block ends first, you're still inside the cell." Perfect symmetry: swap distance ↔ time, δ ↔ τ. Movement between a state and a neighbour = one straight leg at a single fixed speed from 𝒱.

**T23 — Act 3: the node-first Bellman equations, symbol by symbol.** `[status: verified; commit 1c6bbb4]` `[added 2026-07-26]`
- **C*(d,t) = min over predecessors (d̃,t̃) ∈ 𝒮 with (d,t) ∈ 𝒜(d̃,t̃) and v̄ = (d−d̃)/(t−t̃) ∈ 𝒱 of [ C*(d̃,t̃) + (t−t̃)·φ(d̃,t̃;v̄) ]**, with `C*(0,0)=0`.
- Reading it: `min over (d̃,t̃)` = try **every point you could have come from**, keep the smallest total · `(d,t)∈𝒜(d̃,t̃)` = the rule for "could have come from" · `v̄=(d−d̃)/(t−t̃)` = the hop's speed, **derived** (distance ÷ time) · `∈𝒱` = only counts if that speed is physically allowed · `C*(d̃,t̃)` = cheapest fuel to the predecessor (already known) · `(t−t̃)·φ` = this leg's fuel = duration × burn rate.
- **Plain words:** "the cheapest way to reach a point = the cheapest way to reach *some earlier point* + the fuel of the hop. Look back at every candidate, the speed is just the slope, keep the minimum." **The inversion that defines node-first: you no longer choose a speed — you choose which grid point you came from, and the speed falls out.** The old two-case `min_{v∈𝒱}{cases}` is gone; the cases were exactly 𝒜's two halves.
- **V*(d,t) = (d−d̃★)/(t−t̃★)** — the slope of the *winning* leg; recorded per state so the optimal schedule is recovered by backtracking to `(0,0)`. Forward-only: Tal's Eqs. 4–5 untouched; `v̄∈𝒱` lives *inside* the min so Eq. 5 needed no edit.

**T24 — Act 4: §4.2 reconciled with the grid — the logic of each fix.** `[status: verified; commits 72033d9 + 12eab79]` `[added 2026-07-26]`
- **Why surgery:** §4.2 was written when its job was to *invent* the grid; after Acts 1–3 the grid already exists, so §4.2 was re-deriving it and claiming now-false things ("𝒮 has to be made finite").
- **New job description:** "the map is already drawn — **count** it, **walk** only the reachable part, **solve** on what the walk found" (bound → enumerate → sweep).
- **Snap ops recast:** ⟨·⟩_δ/⟨·⟩_τ are no longer *how the discretisation is created* — just the **rounding arithmetic** for landing on §4.1's dots. **|𝒮| bound by counting Eq. 4:** "dots per ruler × number of rulers" — ≤⌈T/τ⌉ per distance line, ≤⌈L/δ⌉ per time line → `|𝒮| = O(M·T/τ + Θ·L/δ)`, `|𝒜| = O(κ·|𝒮|)`.
- **One name per thing:** `d̄/t̄ ≡ d_{𝒟(d)}/t_{𝒯(t)}` (duplicate definition removed); `κ` = neighbours-per-state, freeing `K` for "discrete speed levels" (K^lines / Luo's K^N) — the collision is resolved.
- **Honest claims:** the enumeration = **exactly 𝒜(d,t) filtered by v̄∈𝒱** — *up to the glide-past boundary rule* (a τ-unresolvable distance line is skipped; candidates extend past it — successors Eq. 5 doesn't list; formalism gap flagged for Tal, prep §4-3a). The closure discovers the **reachable subset** of 𝒮, not 𝒮 itself — chessboard analogy: Eq. 4 is every square; the walk visits only the squares a knight from the corner can actually reach.
- **Code ↔ formula:** Algorithm 1's two loops are annotated as the first/second set of Eq. 5 — the pseudocode and the equation now point at each other. Alignment sweep also fixed: preamble "finite" overclaim → "finitely many decision epochs"; `d_i = d_{i−1}+l_i`; orphan `FCR_i` → forward pointer to φ; stale `(d̲,t̲)` cross-ref → predecessor `(d̃,t̃)`.


**T25 — Bounding the discretisation gap: the Wang–Meng school (ε-optimality by sandwiching).** `[status: method identified; application proposed]` `[added 2026-07-28]`
- **The question (brainstorm with Ami):** the optimal speed per subsegment is decided *between two grid nodes* — a better speed may lie between them. Is there a scientific way to measure the distance to a better / lower-bound decision?
- **The school:** **Qiang Meng** (NUS; **editorial board of TR-B** — Tal's flagship venue — and co-EiC of TR-E) + **Shuaian (Hans) Wang** (HK PolyU; co-EiC TR-E, AE TR-B). Their signature: **convexity-certified ε-optimality**. Key papers: **Wang & Meng 2012, "Sailing speed optimization for container ships in a liner shipping network" (TR-E) — already in our bib (`Wang2012`)**; Meng et al. 2019, "Two-phase optimal solutions for ship speed and trim optimization over a voyage using voyage report data" (TR-B 122:88–114) — **candidate to ADD to Related Work**.
- **The method:** leg fuel vs leg *time* `F(t)=φ(ℓ/t)·t ∝ ℓ³/t²` is **convex** → (1) **tangent lines under-estimate** a convex function → outer-approximation LP = **rigorous lower bound**; (2) any feasible plan (our DP) = **upper bound**; (3) refine tangents until `UB−LB ≤ ε` → certificate. Secant (over) + tangent (under) **sandwich** the function; for power-law fuel the max secant error per interval has a **closed form** → grid can be sized *a priori* for a target ε. Answer to "is there a better speed between my nodes?": *you don't find it, you bound it* — `UB−LB` **is** the distance to a better decision.
- **Application to our DP (3 layers):** (a) **a-priori per-leg bound** — per-leg excess ≤ ⅛·F″·(gap)², summed along the optimal path (upgrades T19 from argument to number); (b) **rigorous LB** — relax the endogenous time-block coupling (each cell gets its most favorable straddled block) → legs decouple → exact continuous optimum via **recursive smoothing** (`Norstad2011`, exactness proven in `Hvattum2013` — both already cited!) → report `F_DP − LB`; (c) **empirical certificate** — solve at `(δ,τ)` and `(δ/2,τ/2)`, O(h²) convergence bounds the residual (half-done via the node-first A/B).
- **Why it matters:** if certified gap ≪ SR–Luo gap (1.9–2.8%), the granularity conclusion is provably not grid noise — the exact question a Meng/Wang-school referee would ask. Machinery cites only papers already in the bib + one addition.
- **NB (researcher hunt):** resolved in **T26** — the researcher Ami recalled is **Michael Hewitt** (EiC of Transportation Science), method = **Dynamic Discretization Discovery**. Meng/Wang stay logged as the complementary convexity-side school + Related-Work candidates.


**T26 — CONFIRMED: the researcher is Michael Hewitt; the method is Dynamic Discretization Discovery (DDD).** `[status: identified & confirmed by Ami; application proposed]` `[added 2026-07-28]`
- **Who:** **Michael Hewitt** — **Editor-in-Chief of Transportation Science** (Tal's venue), Professor & endowed chair, Quinlan School of Business, Loyola University Chicago (American). Co-inventor of DDD with **Natashia Boland** & **Martin Savelsbergh**.
- **Key papers (Related-Work candidates, run through /lit-validate before filing):**
  - Boland, Hewitt, Marshall, Savelsbergh (2017), *The Continuous-Time Service Network Design Problem*, **Operations Research 65(5)** — founding DDD paper. DOI 10.1287/opre.2017.1624
  - Marshall, Boland, Savelsbergh, Hewitt (2021), *Interval-Based Dynamic Discretization Discovery for Solving the CTSNDP*, **Transportation Science** — partial networks < 1% of the full expansion. DOI 10.1287/trsc.2020.0994
  - Hewitt (2019), *Enhanced DDD for the Continuous-Time Load Plan Design Problem*, Transportation Science.
  - Boland, Hewitt, Marshall, Savelsbergh (2019), *The price of discretizing time*, EURO J. Transportation & Logistics — literally our question's title.
- **The method (in our language):** decisions are continuous-time; the solvable model is a time-expanded network on discrete nodes — the optimum may lie *between* nodes. DDD: (1) build a **small partial grid that provably under-approximates** — arc arrivals rounded **down** to existing points ("impossibly early" = optimistic) → every continuous solution has a ≤-cost counterpart → the partial model's optimum is a **rigorous LB on the continuous optimum**; (2) **repair** the LB solution into a timing-consistent schedule → feasible **UB** (if already consistent → it IS the optimum); (3) **refine only where the relaxation cheated** (add exactly the missing time points on the shortened arcs); (4) iterate → converges to the **continuous-time optimum with a UB−LB certificate at every step**, typically on < 1% of the full grid. Answer to "is there a better speed between my two nodes?": *DDD converges to the between-the-nodes optimum itself, certifying the gap along the way.*
- **Mapping to our DP:** our 𝒮 (τ=0.1 h, δ=1 NM) *is* the full time-expanded network DDD avoids; F_DP = feasible UB; missing piece = the LB. DDD-for-SR: partial grid = mandatory lines (cell boundaries, 6 h blocks, T — uncheatable, always present) + a few points per line; round arrivals down (same v̄, less time charged → optimistic → valid LB); repair along the LB path with speeds in 𝒱 → UB; refine where the gap concentrates (predictably: cell corners, weather fronts).
- **Three payoffs:** (1) **certificate** that the continuous-𝒱 optimum is within ε of F_DP → the 1.9–2.8 % SR–Luo gap is provably not grid noise (answers the referee question with the TranSci EiC's own method); (2) possible **faster solver** — partial grids ≪ our 1.18M arcs (future-contribution direction); (3) principled non-uniform (δ,τ) refinement.
- **Relation to T25:** complementary — **DDD certifies the time-grid side; Wang–Meng's convexity sandwich certifies the fuel-function side.** DDD is the closer match to the node-granularity framing.
- **Next steps (open):** (a) /lit pipeline on Boland et al. 2017 + Marshall et al. 2021 → pillar filing; (b) sketch a DDD-for-SR prototype (LB via arrival-rounding on a mandatory-lines-only grid, one voyage, measure UB−LB); (c) discuss with Tal — Related Work addition + possibly a §7 "certifying the discretisation" paragraph or future-work note.
